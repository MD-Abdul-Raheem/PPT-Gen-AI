...
const PreviewSection: React.FC<PreviewSectionProps> = ({
  ...
  if (slides.length === 0) {
    return (
      <div className="h-full w-full flex flex-col items-center justify-center text-center p-8 border-2 border-dashed border-slate-300 rounded-3xl bg-white/50">
  ...
  return (
    <div className="flex flex-col h-full gap-6">
      ...
      {/* Top Controls Bar */}
      <div className="flex justify-between items-center bg-white/90 backdrop-blur-sm p-2 rounded-2xl shadow-sm border border-slate-200/50 mx-4 sm:mx-10 mt-2 z-20 flex-wrap gap-2">
      ...
      <div className="flex-1 flex items-center justify-center relative min-h-0">
        <button 
            ...
            className="absolute left-0 sm:left-4 z-20 p-3 bg-white hover:bg-slate-50 rounded-full shadow-float border border-slate-100 text-slate-700 disabled:opacity-0 disabled:pointer-events-none transition-all hover:scale-110 active:scale-95 group"
        >
        ...
        <button 
            ...
            className="absolute right-0 sm:right-4 z-20 p-3 bg-white hover:bg-slate-50 rounded-full shadow-float border border-slate-100 text-slate-700 disabled:opacity-0 disabled:pointer-events-none transition-all hover:scale-110 active:scale-95 group"
        >
            <ChevronRight size={24} className="group-hover:translate-x-0.5 transition-transform" />
        </button>

        <div className="relative w-full max-w-4xl px-4 sm:px-12 md:px-16 perspective-1000">
            <div 
                className="w-full aspect-video shadow-2xl rounded-xl overflow-hidden flex flex-col p-8 sm:p-12 relative transition-transform duration-500 bg-white ring-1 ring-black/5"
                style={{
                     ...slideStyle,
            ...
            >
                ...
                {/* Slide Header */}
                <div className="mb-6 border-b-2 border-transparent relative group min-h-[4rem]">
                    {isEditing ? (
                        <input 
                            value={currentSlide.title}
                            onChange={handleTitleChange}
                            className="w-full text-3xl sm:text-4xl font-bold bg-transparent border-b-2 border-primary-500/20 focus:border-primary-500 outline-none pb-2 placeholder-slate-300 transition-colors"
                            style={titleStyle}
                            placeholder="Slide Title"
                        />
                    ) : (
                        <h2 className="text-3xl sm:text-4xl font-bold pb-4 leading-tight" style={titleStyle}>
                            {currentSlide.title}
                        </h2>
                    )}
                </div>
                            ...
                            {currentSlide.content.map((point, idx) => (
                                <li key={idx} className="flex items-start gap-4 group/item">
                                    ...
                                    {isEditing ? (
                                        ...
                                        <div 
                                            className="text-lg sm:text-xl leading-relaxed opacity-90 font-medium"
                     ...
                     {(currentSlide.imagePrompt || currentSlide.imageData) && (
                         <div className="w-1/3 flex flex-col items-center justify-center">
                             {currentSlide.imageData ? (
                                 ...
                                 <div className="w-full aspect-[4/3] bg-slate-100 rounded-lg border-2 border-dashed border-slate-300 flex flex-col items-center justify-center text-slate-400 gap-3 p-4 text-center">
                ...
                {/* Footer */}
                <div className="absolute bottom-6 right-8 text-sm opacity-50 select-none font-medium tracking-widest">
                    {currentIndex + 1}
                </div>
      ...
      {/* Thumbnails */}
      <div className="h-28 flex items-center gap-3 overflow-x-auto px-6 pb-4 scrollbar-hide flex-shrink-0">
          {slides.map((slide, idx) => (
              <button
                key={slide.id}
                onClick={() => setCurrentIndex(idx)}
                className={`flex-shrink-0 w-36 h-20 rounded-lg border overflow-hidden transition-all duration-300 group relative ${
                  idx === currentIndex 
